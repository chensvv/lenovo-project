<template>
  <div class="table height-105">
    ...
    <el-form :inline="true" ref="searchItem" :model="searchItem" class="demo-form-inline width130 height70" size="mini" @submit.native.prevent style="padding-left:10px;">
      <div class="form-input height70">
        <el-form-item label="说法" prop="regular">
          <el-input v-model.trim="searchItem.regular" clearable @keydown.enter.native="onSubmit"></el-input>
        </el-form-item>
    ...
    <el-dialog :close-on-click-modal="false" :close-on-press-escape="false" title="" :visible.sync="redisVisible" width="40%" top="35vh" @open="openFun('redisList')">
      <el-form :label-position="'right'" label-width="70px" size="small" :rules="redisRules" :model="redisList" ref="redisList" @submit.native.prevent>
    ...
    playVoice(a){
      ...
      if(a == 'add'){
        ...
        // }else{
          ...
          formData = `text=${encodeURI(this.addList.liju)}&user=${Base64.encode('10140802691')}&speed=5&volume=9&pitch=5&speaker=db4`
        // }
      }else{
        // if(proURL.proURL.indexOf('8085') != '-1'){
        ...
        // }else{
          ...
          formData = `text=${encodeURI(this.currentItem.liju)}&user=${Base64.encode('10140802691')}&speed=5&volume=9&pitch=5&speaker=db4`
        // }
      }
      // if(proURL.proURL.indexOf('8085') != '-1'){
      ...
      // }else{
        req.open("POST", 'https://voice.lenovomm.com/website/cloudtts', true);
        req.setRequestHeader('channel', 'cloudasr')
